...
в `<эксперимент>/new_poly`, готовит `<имя>_p.cfx` рядом с исходным и правит в нём две строки:

| строка | было | стало |
|---|---|---|
| `POLY_FILE` | `%W:RA_C.txt` | `%W:new_poly\RA_C.txt` |
| `OUT FILE` | `...V1.uvx` | `...V1_p.uvx` |

Само значение `%W` не трогается — у коррелятора оно своё. Полиномы лежат в подпапке внутри
`%W`, поэтому задание должно указывать туда же; вывод коррелятора остаётся в корне `%W`.
Имя подпапки и каталог, куда пишутся полиномы, задаются одним ключом `--poly-dir`, так что
разъехаться они не могут. Модель при этом не меняется — инструмент только ставит обработчик
метки и вызывает `process_task`.
