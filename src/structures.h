...
// Запрос метки времени для расчёта TIMEOFS (задержка сброса сигнала космос -> пункт приёма).
// Передаётся в обработчик метки времени (см. set_time_mark_hook в functions.h): по этим полям
// посторонний код определяет, для какого файла данных космической станции нужна метка.
struct TimeMarkRequest {
    std::string cfx_path;    // путь к текущему заданию .cfx
    std::string station;     // имя космической станции в задании (напр. RASTRON)
    std::string file_index;  // индекс строки FILExx: "00", "01", ...
    std::string file_value;  // значение строки FILExx как оно записано в задании
};
