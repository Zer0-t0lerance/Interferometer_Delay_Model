...
/**
 * @brief Вычисление теоретической ВАКУУМНОЙ задержки и её производной (порт THEOR_DELAYcorr).
 *
 * Учитывает релятивистскую гравитационную задержку Шапиро (Солнце, Луна, Земля),
 * геометрию с поправками на движение станций (включая члены term2e/term2f для
 * Радиоастрона) и инструментальную задержку смещения оси монтировки (dtau_off).
 * ВНИМАНИЕ: тропосфера и термодеформация здесь НЕ добавляются — тропосферу
 * прибавляет вызывающий код (trop_delay), термодеформация учитывается в координатах
 * станций (THERM_DEF). Это соответствует версии, вызываемой главной программой ARIADNA.
 *
 ...
 * @param[in]  dtau_off    Матрица 2x2: инструментальная задержка оси монтировки (строки - станции, столбцы - задержка/скорость) [с, с/с].
 * @param[out] t2_t1       Теоретическая вакуумная задержка (без тропосферы) [секунды].
...
/**
 * @brief Вычисление матрицы перехода из земной системы координат (ITRF) 
 * в небесную квазиинерциальную систему (J2000) и её первой производной по времени.
 *
 ...
 * @param[out] R2000      Выходная матрица поворота 3x3 (X_J2000 = R2000 * X_ITRF).
 * @param[out] dR2000_dt  Выходная матрица производной поворота 3x3 (V_J2000 = dR2000_dt * X_ITRF).
 */
