 ...
 * геоцентрической задержки в вакууме и пишет файл (имя из POLY_FILE) в out_dir.
 * Эфемериды должны быть инициализированы заранее (init_ephemeris).
 *
 * @param[in] cfx_path   Путь к файлу задания .cfx.
 * @param[in] orbit_path Путь к файлу орбиты .scf. Если "" — орбита берётся из cfx (ORB_FILE);
 *                       непустой путь подставляет свой файл (напр. проверка на своём компьютере).
