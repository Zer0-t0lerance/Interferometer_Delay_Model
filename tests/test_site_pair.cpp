...
int main() {
    ...
    // Каталожные ITRF (Вывод.txt). Дрейф по скорости не применяем (эпоха каталога неизв.).
    SitePrep s1, s2;
    s1.xsta_itrf << 4985370.025, -3955020.358, -428472.184;  // FORTLEZA
    s2.xsta_itrf << 5085490.783,  2668161.315, -2768692.721;  // HART15M
    ...
    // Океан/полюс/атм отключены (изоляция твёрдого прилива)
    ...
    site_pair(s1, s2, 58120, 0.7088, jd0, ut1_sec, cent, f, fd, gast, sun_geo, moon_geo,
              0.0, 0.0, 0.0, 0.0, r2000, dr2000, d2r2000, xs, vs, as_);
    ...
    // СТРУКТУРНАЯ проверка: site_pair собран и даёт координаты рядом с дампом.
    // Невязка (~дециметры) доминируется ДРЕЙФОМ координат по скорости (эпоха каталога
    // неизвестна -> не применили +v*dyear) и пропущенными океан/полюс/атм (нужны EOP).
    // Метровая/мм-сверка закроется с точной эпохой каталога и EOP.
    bool ok = df < 0.5 && dh < 0.5;
    printf("---------------------------------------------------------------------\n");
    printf("  site_pair собран, координаты в пределах ~0.5 м от дампа: %s\n", ok ? "OK" : "FAIL");
    printf("  (остаток = дрейф v*dyear [эпоха каталога неизв.] + океан/полюс/атм [нужен EOP])\n");
    printf("  РЕЗУЛЬТАТ: %s\n", ok ? "PASS" : "FAIL");
    return ok ? 0 : 1;
}
